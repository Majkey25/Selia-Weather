...
from aladin_ensemble.sources.live_capture import CaptureError, compare_capture_models
...
def test_replay_baselines_share_the_complete_hour_mask() -> None:
    ...
    assert compare_capture_models((first, second, incomplete)) == {"temperature": {
...
def test_replay_cannot_hide_disagreement_about_the_matched_truth() -> None:
    ...
    with pytest.raises(ValueError, match="disagree"):
        compare_capture_models((first, replace(first, model_id="b", observed_value=13)))
...
def test_replay_does_not_compute_a_linear_average_of_directions() -> None:
    ...
    assert compare_capture_models((first,)) == {"wind_direction": {
